...
export async function publish() {
  ...
  await pushToKnowledgebase(knowledgeJson, graphJson);
}
...
async function pushToKnowledgebase(knowledgeJson, graphJson) {
  const url = `https://api.github.com/repos/${KNOWLEDGEBASE_REPO}/contents/knowledge.json`;
  const graphUrl = `https://api.github.com/repos/${KNOWLEDGEBASE_REPO}/contents/knowledge.graph.json`;

  const knowledgeCommit = await commitFile(url, knowledgeJson, "knowledge.json");
  const graphCommit = await commitFile(graphUrl, graphJson, "knowledge.graph.json");
