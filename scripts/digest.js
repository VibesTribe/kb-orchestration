...
export async function digest() {
  ...
  // ------------------ Fallback handling ------------------
  if (!digestPayload.projects.some((p) => p.high.length || p.moderate.length)) {
    log("No highly or moderately useful items; sending fallback digest");
    await sendBrevoEmail({
      subject: "Daily Digest – No useful updates today",
      textContent: renderFallbackTextDigest(digestPayload),
      htmlContent: renderFallbackHtmlDigest(digestPayload),
      recipients,
    });
    return;
  }
...
function collectItemsForProject(curated, project) {
  ...
  for (const item of curated.items ?? []) {
    const assignment =
      ...
      (item.assignedProjects ?? []).find(
        (entry) => entry.projectKey === project.key || entry.project === project.name
      );
    if (!assignment) continue;
    if (assignment.usefulness?.toLowerCase() === "high")
      high.push(buildDigestEntry(item, assignment));
    else if (assignment.usefulness?.toLowerCase() === "moderate")
...
function renderFallbackTextDigest(payload) {
  const dateStr = new Date(payload.generatedAt).toLocaleDateString("en-US", {
    year: "numeric",
    month: "long",
    day: "numeric",
...
No useful updates today.

Stay calm and build on.`;
}
function renderFallbackHtmlDigest(payload) {
  const dateStr = new Date(payload.generatedAt).toLocaleDateString("en-US", {
    year: "numeric",
    month: "long",
    day: "numeric",
  ...
  <h1>Daily Digest</h1>
  <p>${dateStr}</p>
  <p>No useful updates today.</p>
  <p><em>Stay calm and build on.</em></p>
  <p><a href="https://vibestribe.github.io/kb-site/">View this digest online</a></p>
  </body></html>`;
}
